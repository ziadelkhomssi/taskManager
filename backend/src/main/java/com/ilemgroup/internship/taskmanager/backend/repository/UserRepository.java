...
public interface UserRepository extends JpaRepository<User, String> {
    ...
    @Query("""
            SELECT us FROM User us
            WHERE LOWER(us.job) LIKE LOWER(CONCAT('%', ?1, '%'))
    """)
    ...
    @Query("""
            ...
            WHERE pr.id=?1 AND LOWER(us.name) LIKE LOWER(CONCAT('%', ?2, '%'))
    """)
    ...
    @Query("""
            ...
            LEFT JOIN Sprint sp ON sp.id=ti.sprint.id
            LEFT JOIN Project pr ON pr.id=sp.project.id
            WHERE pr.id=?1 AND LOWER(us.job) LIKE LOWER(CONCAT('%', ?2, '%'))
    """)
    ...
    @Query("""
            ...
            LEFT JOIN Ticket ti ON ti.user.azureOid=us.azureOid
            LEFT JOIN Sprint sp ON sp.id=ti.sprint.id
            WHERE sp.id=?1 AND LOWER(us.name) LIKE LOWER(CONCAT('%', ?2, '%'))
    """)
    ...
    @Query("""
            ...
            LEFT JOIN Ticket ti ON ti.user.azureOid=us.azureOid
            LEFT JOIN Sprint sp ON sp.id=ti.sprint.id
            WHERE sp.id=?1 AND LOWER(us.job) LIKE LOWER(CONCAT('%', ?2, '%'))
    """)
